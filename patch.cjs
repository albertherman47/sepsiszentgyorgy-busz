...
const minHeapCode = `class MinHeap<T> {
  private heap: T[] = [];
...
code = code.replace(/export function planTrip/, minHeapCode + '\nexport function planTrip');
code = code.replace(/const queue: SearchState\[\] = \[\];/, 'const queue = new MinHeap<SearchState>((a, b) => a.availableAt.getTime() - b.availableAt.getTime());');
code = code.replace(/queue\.sort\(\(a, b\) => a\.availableAt\.getTime\(\) - b\.availableAt\.getTime\(\)\);/, '');
