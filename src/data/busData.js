...
// MEGÁLLÓK
// ============================================================
export const stops = Object.values(ALL_STOPS).map((baseStop) => {
    const lineIds = registeredLines
        .filter((line) => line.stopIds.includes(baseStop.id))
        .map((line) => line.id);
    return {
        ...baseStop,
        lineIds,
...
// JÁRATOK
// ============================================================
export const lines = registeredLines.map((line) => ({
    id: line.id,
    number: line.number,
    ...
    paths: line.roadPaths,
    directionPaths: line.directionPaths,
    directionStopIds: { outbound: line.outboundStopIds, return: line.returnStopIds },
    path: line.roadPath ||
        line.roadPaths?.[0] ||
        line.stopIds
            .map((stopId) => {
